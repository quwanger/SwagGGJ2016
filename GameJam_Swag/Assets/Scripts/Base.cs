...
public class Base : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D collider) {
		if (!gameManager.inGodMode) {
			...
			else if(collider.gameObject.GetComponent<MapleLeaf> () && gameManager.throwToScore)
			{
				if (collider.gameObject.GetComponent<MapleLeaf> ().thrower != null) {
					...
					if (player.PlayerId == this.playerId) {
						if (collider.gameObject.GetComponent<MapleLeaf> ().leafColor == player.activeColor) {
							...
							vulnerableGem = collider.gameObject;
							player.gameManager.spawnManager.leavesOnMap.Remove (collider.gameObject);
							collider.gameObject.transform.parent = this.transform;
							collider.gameObject.GetComponent<CircleCollider2D> ().isTrigger = false;
							collider.gameObject.GetComponent<MapleLeaf> ().StopLeaf ();
							collider.gameObject.GetComponent<MapleLeaf> ().captor = player;
